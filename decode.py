...
class Decode:
    ...
    def decode(self, img):
        ...
        for i in range(img.shape[0]-1,0,-1):
            try:
                ...
                ean13, is_valid = Decode.decode_line(thresh[i])
    ...
    def decode_line(line):
        bars = Decode.read_bars(line)
        left_guard, left_patterns, center_guard, right_patterns, right_guard = Decode.classify_bars(bars)
        Decode.convert_patterns_to_length(left_patterns)
        Decode.convert_patterns_to_length(right_patterns)
        left_codes = Decode.read_patterns(left_patterns,is_left=True)
        right_codes = Decode.read_patterns(right_patterns,is_left=False)
        ean13 = Decode.get_ean13(left_codes,right_codes)
        #print("Detected code: "+ ean13)
        is_valid = Decode.verify(ean13)
    ...
    def read_patterns(patterns,is_left=True):
        ...
        for i in range(6):
            ...
            tmp1=(m1+m2)*1.0;
            tmp2=(m2+m3)*1.0;
            at1 = Decode.get_AT(tmp1/total)
            at2 = Decode.get_AT(tmp2/total)
            if is_left:
                decoded = Decode.decode_left(at1,at2,m1,m2,m3,m4)
            else:
                decoded = Decode.decode_right(at1,at2,m1,m2,m3,m4)
    ...
    def get_ean13(left_codes,right_codes):
        ean13 = ""
        ean13 = ean13 + str(Decode.get_first_digit(left_codes))
